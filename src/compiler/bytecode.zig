...
pub const OpCode = enum(u8) {
    ...
    closure_multi = 0x69,
    /// partial 作成（オペランド: 引数の数 u16）
    /// スタック上に [関数, arg1, arg2, ...] がある状態
    partial = 0x6A,
    /// comp 作成（オペランド: 関数の数 u16）
    /// スタック上に [f1, f2, f3, ...] がある状態（実行は右から左）
    comp = 0x6B,
    /// reduce（オペランド: フラグ u16 - 1 なら初期値あり、0 なら初期値なし）
    /// 初期値あり: スタック上に [関数, 初期値, コレクション]
    /// 初期値なし: スタック上に [関数, コレクション]
    reduce = 0x6C,
    /// map（スタック上に [関数, コレクション]）→ 変換済みリスト
    map_seq = 0x6D,
    ...
    // ═══════════════════════════════════════════════════════════
    // [K2] Atom 操作 (0xB0-0xBF)
    // ═══════════════════════════════════════════════════════════
    /// swap!（オペランド: 追加引数の数 u16）
