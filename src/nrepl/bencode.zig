...
//!
//! nREPL ワイヤーフォーマット。
//! - 文字列: `<len>:<data>` (例: `5:hello`)
//! - 整数:   `i<num>e` (例: `i42e`)
//! - リスト: `l<items>e`
//! - 辞書:   `d<key><val>...e` (キーは文字列)
...
/// ストリームから bencode 値を1つデコード
/// バッファから読み取り、消費したバイト数を返す
...
// テスト用 arena ヘルパー (decode が内部で allocate するため)
fn testArena() std.heap.ArenaAllocator {
    return std.heap.ArenaAllocator.init(std.testing.allocator);
}

test "bencode encode/decode 文字列" {
    var arena = testArena();
    defer arena.deinit();
...
test "bencode encode/decode リスト" {
    var arena = testArena();
    defer arena.deinit();
    const allocator = arena.allocator();
    var buf: std.ArrayListUnmanaged(u8) = .empty;
