//! バックトラッキング正規表現マッチャー
//!
//! コンパイル済み RegexNode AST を直接走査してマッチングする。
//! Java/Python/Ruby と同じバックトラッキング方式。
...
pub const MatchResult = struct {
    ...
    /// キャプチャグループ: groups[0] = 全体, groups[1..] = 各グループ
    groups: []const ?Span,
};
...
pub const Matcher = struct {
    ...
    fn matchAnchor(self: *const Matcher, a: regex_mod.Anchor, pos: usize) ?usize {
        ...
        return if (matches) pos else null; // アンカーは位置を消費しない
    }
    ...
    /// 量指定子マッチ（greedy のみ。lazy は tryMatchNodesAt 経由で処理）
    fn matchQuantifier(self: *Matcher, q: regex_mod.Quantifier, pos: usize) MatchError!?usize {
        // lazy quantifier がここに来た場合（グループ内等）は greedy として処理
        // 正確な lazy バックトラックは tryMatchNodesAt が担当
    ...
    fn matchQuantifierGreedy(self: *Matcher, q: regex_mod.Quantifier, pos: usize) MatchError!?usize {
        ...
        while (count < max_count) {
            if (try self.tryMatchNode(q.child.*, current_pos)) |new_pos| {
                if (new_pos == current_pos) break; // 無限ループ防止
                current_pos = new_pos;
                count += 1;
...
test "リテラルマッチ" {
    ...
    try std.testing.expectEqualStrings("abc", "xabcy"[result.?.start..result.?.end]);
}
...
test "キャプチャグループ" {
    ...
    // groups[0] = 全体
    try std.testing.expect(result.?.groups[0] != null);
    try std.testing.expectEqualStrings("12-34", result.?.groups[0].?.text("x12-34y"));

    // groups[1] = "12"
    try std.testing.expect(result.?.groups[1] != null);
    try std.testing.expectEqualStrings("12", result.?.groups[1].?.text("x12-34y"));

    // groups[2] = "34"
    try std.testing.expect(result.?.groups[2] != null);
...
test "文字クラス [a-z]" {
    ...
    const compiled = try compile(allocator, "[a-z]+");
    const result = try findFirst(allocator, &compiled, "123abc456");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("abc", "123abc456"[result.?.start..result.?.end]);
}
...
test "否定文字クラス [^0-9]" {
    ...
    const compiled = try compile(allocator, "[^0-9]+");
    const result = try findFirst(allocator, &compiled, "123abc456");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("abc", "123abc456"[result.?.start..result.?.end]);
}
...
test "選択 a|b" {
    ...
    const compiled = try compile(allocator, "cat|dog");

    const r1 = try findFirst(allocator, &compiled, "I have a cat");
    try std.testing.expect(r1 != null);
    try std.testing.expectEqualStrings("cat", "I have a cat"[r1.?.start..r1.?.end]);

    const r2 = try findFirst(allocator, &compiled, "I have a dog");
    try std.testing.expect(r2 != null);
    try std.testing.expectEqualStrings("dog", "I have a dog"[r2.?.start..r2.?.end]);
}
...
test "アンカー ^ $" {
    ...
    const compiled = try compile(allocator, "^abc$");
    try std.testing.expect(try isFullMatch(allocator, &compiled, "abc"));
    try std.testing.expect(!try isFullMatch(allocator, &compiled, "xabc"));
    try std.testing.expect(!try isFullMatch(allocator, &compiled, "abcx"));
}
...
test "\\w+ マッチ" {
    ...
    const compiled = try compile(allocator, "\\w+");
    const result = try findFirst(allocator, &compiled, "  hello  ");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("hello", "  hello  "[result.?.start..result.?.end]);
}
...
test "\\s+ マッチ" {
    ...
    const compiled = try compile(allocator, "\\s+");
    const result = try findFirst(allocator, &compiled, "abc  def");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("  ", "abc  def"[result.?.start..result.?.end]);
}
...
test "量指定子 {2,4}" {
    ...
    const compiled = try compile(allocator, "a{2,4}");
    try std.testing.expect((try findFirst(allocator, &compiled, "a")) == null);

    const r2 = try findFirst(allocator, &compiled, "aa");
    try std.testing.expect(r2 != null);
    try std.testing.expectEqualStrings("aa", "aa"[r2.?.start..r2.?.end]);

    const r5 = try findFirst(allocator, &compiled, "aaaaa");
    try std.testing.expect(r5 != null);
    // 貪欲: 最大4つマッチ
    try std.testing.expectEqualStrings("aaaa", "aaaaa"[r5.?.start..r5.?.end]);
}
...
test "word boundary \\b" {
    ...
    const compiled = try compile(allocator, "\\bfoo\\b");
    try std.testing.expect((try findFirst(allocator, &compiled, "foo bar")) != null);
    try std.testing.expect((try findFirst(allocator, &compiled, "foobar")) == null);
}
...
test "case insensitive (?i)" {
    ...
    const compiled = try compile(allocator, "(?i)abc");
    try std.testing.expect((try findFirst(allocator, &compiled, "ABC")) != null);
    try std.testing.expect((try findFirst(allocator, &compiled, "AbC")) != null);
}
...
test "後方参照 (a)\\1" {
    ...
    const compiled = try compile(allocator, "(\\w+)\\s+\\1");
    const result = try findFirst(allocator, &compiled, "hello hello");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("hello hello", "hello hello"[result.?.start..result.?.end]);

    try std.testing.expect((try findFirst(allocator, &compiled, "hello world")) == null);
}
...
test "先読み (?=...)" {
    ...
    const compiled = try compile(allocator, "\\w+(?=\\s)");
    const result = try findFirst(allocator, &compiled, "hello world");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("hello", "hello world"[result.?.start..result.?.end]);
}
...
test "否定先読み (?!...)" {
    ...
    const compiled = try compile(allocator, "foo(?!bar)");
    try std.testing.expect((try findFirst(allocator, &compiled, "foobaz")) != null);
    try std.testing.expect((try findFirst(allocator, &compiled, "foobar")) == null);
}
...
test "空パターンはどこでもマッチ" {
    ...
    const result = try findFirst(allocator, &compiled, "abc");
    try std.testing.expect(result != null);
...
test "メタ文字エスケープ \\." {
    ...
    const compiled = try compile(allocator, "\\d+\\.\\d+");
    const result = try findFirst(allocator, &compiled, "pi=3.14");
    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("3.14", "pi=3.14"[result.?.start..result.?.end]);
}
...
test "(\\w+)@(\\w+) キャプチャ" {
    ...
    const compiled = try compile(allocator, "(\\w+)@(\\w+)");
    const input = "email: user@host";
    const result = try findFirst(allocator, &compiled, input);

    try std.testing.expect(result != null);
    try std.testing.expectEqualStrings("user@host", input[result.?.start..result.?.end]);
    try std.testing.expectEqualStrings("user", result.?.groups[1].?.text(input));
    try std.testing.expectEqualStrings("host", result.?.groups[2].?.text(input));
}

test "連続マッチ (find multiple)" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
